...
module.exports = function addContributor(options, username, contributions) {
  ...
  const contributorsP = answersP.then(answers => add(options, answers.username, answers.contributions, repo.getUserInfo));
  const writeContributorsP = contributorsP.then(contributors => util.configFile.writeContributors(options.config, contributors));
  return Promise.all([answersP, contributorsP, writeContributorsP]).then(res => {
    const answers = res[0];
