...
const contributionChoices = _.flow(util.contributionTypes, _.toPairs, _.sortBy(pair => {
...
function getQuestions(options, username, contributions) {
  return [{
    ...
    default: function (answers) {
      ...
      return options.contributors.filter(entry => entry.login && entry.login.toLowerCase() === answers.username.toLowerCase()).reduce((allEntries, entry) => allEntries.concat(entry.contributions), []);
    },
    choices: contributionChoices(options),
    validate: function (input, answers) {
      answers.username = answers.username || username;
      const previousContributions = options.contributors.filter(entry => entry.login && entry.login.toLowerCase() === answers.username.toLowerCase()).reduce((allEntries, entry) => allEntries.concat(entry.contributions), []);
      if (!input.length) {
...
function getValidUserContributions(options, contributions) {
  ...
  const validUserContributions = _.filter(userContribution => validContributionTypes[userContribution] !== undefined)(userContributions);
  const invalidUserContributions = _.filter(userContribution => validContributionTypes[userContribution] === undefined)(userContributions);
