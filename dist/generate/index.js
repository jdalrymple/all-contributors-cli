...
function generateContributorsList(options, contributors) {
  ...
  return _.flow(_.sortBy(contributor => {
    if (options.contributorsSortAlphabetically) {
      return contributor.name;
    }
  }), _.map(function formatEveryContributor(contributor) {
    return formatContributor(options, contributor);
  }), _.chunk(options.contributorsPerLine), _.map(currentLineContributors => formatLine(options, currentLineContributors)), _.join(seperator), newContent => {
    if (options.linkToUsage) {
