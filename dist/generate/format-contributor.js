...
const avatarTemplate = _.template('<img src="<%= contributor.avatar_url %>?s=<%= options.imageSize %>" width="<%= options.imageSize %>px;" alt="<%= name %>"/>');
const avatarBlockTemplate = _.template('<a href="<%= contributor.profile %>"><%= avatar %><br /><sub><b><%= name %></b></sub></a>');
const avatarBlockTemplateNoProfile = _.template('<%= avatar %><br /><sub><b><%= name %></b></sub>');
const contributorTemplate = _.template('<%= avatarBlock %><br /><%= contributions %>');
const defaultImageSize = 100;
function defaultTemplate(templateData) {
  const rawName = templateData.contributor.name || templateData.contributor.login;
  const name = escapeName(rawName);
  const avatar = avatarTemplate(_.assign(templateData, {
    name
  }));
...
module.exports = function formatContributor(options, contributor) {
  ...
  const customTemplate = options.contributorTemplate && _.template(options.contributorTemplate);
