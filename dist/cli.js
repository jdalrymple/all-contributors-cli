...
function startGeneration(argv) {
  return Promise.all(argv.files.map(file => {
...
function checkContributors(argv) {
  ...
  return repo.getContributors(configData.projectOwner, configData.projectName, configData.repoType, configData.repoHost).then(repoContributors => {
    ...
    const knownContributors = configData.contributors.map(contributor => contributor[checkKey]);
    const missingInConfig = repoContributors.filter(key => !knownContributors.includes(key));
